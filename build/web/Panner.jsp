...
<body>
	<header id="header" class="header fixed-top d-flex align-items-center">
		<div class="d-flex align-items-center justify-content-between">
			<a href="MainController" class="logo d-flex align-items-center">
		...
		<nav class="header-nav ms-auto">
			<ul class="d-flex align-items-center">
				...
				<c:if test="${sessionScope.user == null}">
					<a class="btn btn-outline-primary" style="margin-right: 15px;"
						href="LoginController" role="button">Login</a>
					<a class="btn btn-outline-primary" href="RegisterController"
						role="button">Register</a>
				</c:if>

				<c:if test="${sessionScope.user != null}">
					<a class="nav-link nav-icon" href="AddToCartController"> <i class="bi bi-cart"></i>
						<c:if test="${sessionScope.order.getTotalQuantity() > 0}">
							<span class="badge bg-primary badge-number">${sessionScope.order.getTotalQuantity()}</span>
						</c:if>
					</a>
					<!-- End Cart Icon -->
